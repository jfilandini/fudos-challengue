...
RSpec.describe Challenge::UseCases::ProcessDueJobs do
  ...
  def enqueue(run_at: clock.now)
    jobs.create(requested_by_user_id: "1", idempotency_key: "request-1", id: "job-1", product_id: "product-1", product_name: "Laptop", run_at: run_at, now: clock.now)
  end
  ...
  it "creates the product, completes the due job and counts it as processed" do
    ...
    expect(products.find_for_user("product-1", requested_by_user_id: "1")).to have_attributes(name: "Laptop")
  ...
  it "leaves a job untouched until its delay has elapsed" do
    ...
    expect(products.find_for_user("product-1", requested_by_user_id: "1")).to be_nil
  end
  ...
  it "creates the product once the delay has elapsed" do
    ...
    expect(products.find_for_user("product-1", requested_by_user_id: "1")).not_to be_nil
  end
