...
RSpec.describe Challenge::Persistence::JobRepository, "job claiming and transitions" do
  ...
  def create(id, run_at: now, created_at: now)
    jobs.create(id: id, requested_by_user_id: "1", idempotency_key: id, product_id: "product-#{id}", product_name: "Coffee", run_at: run_at, now: created_at)
  end
  ...
  it "claims eligible jobs by due time, creation time and ID" do
    ...
    expect(jobs.find_for_user("future", requested_by_user_id: "1")).to be_pending
  end
  ...
  it "commits in_progress and does not claim the same job twice" do
    ...
    expect(jobs.find_for_user("one", requested_by_user_id: "1")).to eq(claimed)
    expect(jobs.claim_next(now)).to be_nil
  end
  ...
  it "only completes or fails in_progress jobs and preserves terminal states" do
    ...
    expect(jobs.find_for_user("one", requested_by_user_id: "1")).to be_pending
    jobs.claim_next(now)
    jobs.mark_completed("one", now)
    expect(jobs.mark_failed("one", now)).to be(false)
    expect { jobs.mark_completed("one", now) }
      .to raise_error(Challenge::Persistence::JobRepository::InvalidTransition)
    expect(jobs.find_for_user("one", requested_by_user_id: "1")).to be_completed
    ...
    expect { jobs.mark_completed("two", now) }
      .to raise_error(Challenge::Persistence::JobRepository::InvalidTransition)
    expect(jobs.find_for_user("two", requested_by_user_id: "1")).to be_failed
  end
  ...
  it "holds back a job whose scheduled time is still in the future" do
    ...
    expect(jobs.claim_next(now)).to be_nil
    expect(jobs.find_for_user("job-1", requested_by_user_id: "1")).to be_pending
  ...
  it "does not claim a completed job" do
    ...
    expect(jobs.find_for_user("job-1", requested_by_user_id: "1")).to be_completed
  end
  ...
  it "does not claim a failed job" do
    ...
    expect(jobs.claim_next(now + 1)).to be_nil
    expect(jobs.find_for_user("job-1", requested_by_user_id: "1")).to be_failed
  end
  ...
  it "records when a job last changed" do
    ...
    expect(jobs.find_for_user("job-1", requested_by_user_id: "1").updated_at).to eq(now + 1)
  end
end
